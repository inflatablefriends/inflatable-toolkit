...
namespace IF.Common.Metro.Structures
{
    ...
    /// </code>
    /// </remarks>
    public class ListCollectionView<T> : ICollectionViewEx<T>, IEditableCollectionView, INotifyPropertyChanged, IComparer<T> where T : class
    {
        //------------------------------------------------------------------------------------
        #region ** fields

        IEnumerable<T> _source;                                 // original data source
        IList<T> _sourceList;                              // original data source as list
        Type _itemType;                                 // type of item in the source collection
        INotifyCollectionChanged _sourceNcc;            // listen to changes in the source
        List<T> _view;                             // filtered/sorted data source
        ObservableCollection<SortDescriptionAsync<T>> _sort;    // sorting parameters
        Dictionary<string, PropertyInfo> _sortProps;    // PropertyInfo dictionary used while sorting
        Predicate<T> _filter;                      // filter
        ...
        public ListCollectionView(IEnumerable<T> source)
        {
            // view exposed to consumers
            _view = new List<T>();

            // sort descriptor collection
            _sort = new ObservableCollection<SortDescriptionAsync<T>>();
            ...
            if (_updating <= 0)
            {
                switch (e.Action)
                {
                    case NotifyCollectionChangedAction.Add:
                        ...
                        {
                            HandleItemAdded(e.NewStartingIndex, e.NewItems[0] as T);
                        }
                        else
                        {
                            HandleSourceChanged();
                    ...
                    case NotifyCollectionChangedAction.Remove:
                        ...
                        {
                            HandleItemRemoved(e.OldStartingIndex, e.OldItems[0] as T);
                        }
                        else
                        {
                            HandleSourceChanged();
        ...
        public IList<SortDescriptionAsync<T>> SortDescriptions { get { return _sort; } }
        public IEnumerable<T> SourceCollection { get { return _source; } }
        ...
        public int IndexOf(object item) { return _view.IndexOf(item as T); }
        public bool MoveCurrentTo(object item) { return item != CurrentItem ? MoveCurrentToIndex(IndexOf(item as T)) : true; }
        ...
        public void Insert(int index, object item)
        {
            ...
            _sourceList.Insert(index, item as T);
        }
        ...
        public object this[int index]
        {
            ...
            set { _view[index] = value as T; }
        }
        public bool Contains(object item) { return _view.Contains(item as T); }
        public void CopyTo(object[] array, int arrayIndex) { _view.CopyTo(array as T[], arrayIndex); }
        ...
        {
            // this isn't needed with explicit GetProperty Action<T>
            ...
            // compare two items
            foreach (var sortDescription in _sort)
            {
                SortDescriptionAsync<T> sd = sortDescription;
                IComparable cx = null, cy = null;

                Task.Run(async () =>
                {
                    cx = await sd.GetProperty(x) as IComparable;
                }).Wait();
                Task.Run(async () => cy = await sd.GetProperty(y) as IComparable).Wait();
